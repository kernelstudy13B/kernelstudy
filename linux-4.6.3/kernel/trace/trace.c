 ...
 * of pages allocated for its buffer.
 global_trace : live tracing을 위해 트레이싱 버퍼를 홀드하는 지시자.
 각 CPU를 위해 global_trace는 트레이스 엔트리들을 저장할 페이지 연결리스트를 포함한다.
 메모리에서 그 페이지들의 페이지 지시자는 버퍼 per cpu내에 각각의 페이지들로의 페이지 지시자에 있는 lru item을 연결함으로서 그 연결리스트를 홀드하기 위해 사용
 */
...
static cycle_t buffer_ftrace_now(struct trace_buffer *buf, int cpu)
{
	...
	return ts;
	//ring buffer 타임 스탬프를 지정하고 그를 normalize하는 함수
}
...
void tracing_reset_online_cpus(struct trace_buffer *buf)
{
	...
	ring_buffer_record_disable(buffer);//버퍼로의 쓰기를 disable 시킴

	/* Make sure all commits have finished */
	//모든 commit이 끝남을 확실히 함
	synchronize_sched();
...
void trace_printk_init_buffers(void)
{
	...
	pr_warn("**   NOTICE NOTICE NOTICE NOTICE NOTICE NOTICE NOTICE   **\n");
	pr_warn("**********************************************************\n");
	//trace_printk 사용중, extra memory 할당중
	//이 경고문은 현재 디버그 커널에 진입했으며 production use에 안전하지 않다는것을 의미
	//커널 디버깅중이 아닌데 이 메세지가 발견되면 vendor에게 보고
	/* Expand the buffers to set size */
	tracing_update_buffers();

	buffers_allocated = 1;//버퍼가 확실히 할당이 됨을 명시
	 ...
	 * allocated here, then this was called by module code.
	 현재 함수는 모듈들에 의해 불려질수 있다. 모듈에 의해 불려지면 우리는 cmdline 레코딩을 직접 시작해야한다. 만약 global_trace.buffer가 이미 할당이 됐다면 모듈코드에 의해 불리어질수 있다.
	 */
 ...
 */
//링 버퍼를 확장하기 위해 tracing facility에 의해 사용됨
//트레이싱이 시스템이 사용되지 않을때 메모리를 절약하기 위한 것
//링 버퍼는 최소 사이즈로 세팅되지만 유저가 tracing facility를 사용하기 시작하면 디폴트사이즈에서 점점 사이즈가 커진다.
	...
	//여기서 trace_array는 global_trace
{
	...
	tr->clock_id = i;
	//ring_buffer : trace array 안에 trace_buffer안에 있고 그 버퍼안에 링버퍼가 포함
	//tr->trace_buffer.buffer 가 링버퍼
	 ...
	 */
	//새로운 클록은 이전의 클록과 일관적이지 않을수 있다.
	//비교할수 없는 타임스탬프를 가지지 않도록 하기위해 버퍼를 리셋
	tracing_reset_online_cpus(&tr->trace_buffer);
...
__init static int tracer_alloc_buffers(void)
{
	 ...
	 * than we have bits for.
	 우리가 가진 bit보다 더 많은 트레이스 옵션을 더하지 않을 것
	 */
	BUILD_BUG_ON(TRACE_ITER_LAST_BIT > TRACE_FLAGS_MAX_SIZE);

	if (!alloc_cpumask_var(&tracing_buffer_mask, GFP_KERNEL))
		goto out;
	//struct cpumask를 위한 할당

	if (!alloc_cpumask_var(&global_trace.tracing_cpumask, GFP_KERNEL))
		goto out_free_buffer_mask;

	/* Only allocate trace_printk buffers if a trace_printk exists */
	//trace_printk가 존재하면 trace_printk buffers만을 할당
	if (__stop___trace_bprintk_fmt != __start___trace_bprintk_fmt)
		/* Must be called before global_trace.buffer is allocated */
		trace_printk_init_buffers();
	//trace_prink : 디버깅 전용, production 중에 조작을 하면 안됨

	/* To save memory, keep the ring buffer size to its minimum */
	//메모리 절약을 위해 링버퍼 사이즈를 최소로 킵한다
	if (ring_buffer_expanded)
	...
	//현재 cpu의 mask 정보를 tracing의 mask를 위한 필드들에 카피

	raw_spin_lock_init(&global_trace.start_lock);
	//global_trace에 있는 start_lock 필드 초기화
...
void __init trace_init(void)
{
	if (tracepoint_printk) {
		//추측 : tracepoint_printk는 KERNEL_CONFIG 활성화에 따라 1이 되냐 안되냐가 정해지는것으로 예상 됨
		tracepoint_print_iter =
			kmalloc(sizeof(*tracepoint_print_iter), GFP_KERNEL);
		//tracepoint_print_iter 구조체에 대한 공간 할당
		if (WARN_ON(!tracepoint_print_iter))
			tracepoint_printk = 0;
	}
	tracer_alloc_buffers();//trace용 per_cpu 버퍼를 할당받고 초기화
	trace_event_init();
}
