...
asmlinkage __visible void __init start_kernel(void) //called by head.S 
{
	 ...
	 * 프로세서 안의 어떤 코어가 이 커널을 초기화 하는지 세팅.
	*/

	debug_objects_early_init();//디버깅을 위한 자료구조들을 초기화 하는 함수.

	/*
	 * Set up the the initial canary ASAP:
	 */
	boot_init_stack_canary();
	//스택 오버플로우 공격 감지. canary가 그것을 감지하는 변수.

	cgroup_init_early();
	//프로세스를 그룹화 하는 방법 제공.
	//cgroup에 대한 초기화는 나중에 cgroup_init()으로 한번더 하게 된다.

	local_irq_disable();//irq 익셉션을 disable하는매크로
